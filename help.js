...
(() => {
    ...
    const topics = [
        ...
        {id: 'invites', title: ['Контакты и привязка к боту', 'Contacts and bot connections'], items: [
            ...
            ['Кнопка «Написать» сначала открывает Telegram. Если его нет — чат WhatsApp по номеру. Если указан только MAX, TEMLI копирует контакт и открывает установленное приложение MAX: вставьте контакт в поиск. Для ученика и родителя эта последовательность работает отдельно.', 'Write opens Telegram first. If it is missing, it opens a WhatsApp chat by phone number. If only MAX is available, TEMLI copies the contact and opens the installed MAX app: paste it into search. This priority is applied separately to the student and parent.'],
