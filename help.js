...
(() => {
    ...
    const topics = [
        ...
        {id: 'lessons', title: ['Занятия: создание, перенос, отмена', 'Lessons: create, move, cancel'], items: [
            ...
            ['Контакты ученика и родителя можно заполнить в форме индивидуального занятия. Если «Мой бот» подключён, отметьте нужные приглашения — после сохранения появятся ссылки для копирования. Участников группы подключайте через их карточки.', 'Enter student and parent contacts in the individual lesson form. If My bot is connected, select invitations; links appear after saving. Connect group members through their individual profiles.'],
            ...
            ['«Перенести»: выберите свободный прямоугольник в календаре, затем перенос одного, перенос будущих или добавление копии. Занятые места приглушены; подсказка не перекрывает таблицу. «Отмена» в режиме выбора прекращает перенос.', 'Reschedule: select a free rectangle, then move one lesson, move future lessons or add a copy. Busy slots are dimmed; the hint stays outside the grid. Cancel in selection mode exits rescheduling.'],
        ...
        {id: 'invites', title: ['Контакты и привязка к боту', 'Contacts and bot connections'], items: [
            ['Сначала «Настройки» → «Мой бот»: создайте отдельного бота через @BotFather и подключите его токен. Не используйте бота, работающего в другом сервисе; токен никому не пересылайте.', 'First open Settings → My bot: create a dedicated bot via @BotFather and connect its token. Do not use a bot running in another service; keep the token private.'],
            ['Приглашения доступны при создании индивидуального урока и в карточке ученика → «Пригласить в моего бота»: «Ссылка ученику» / «Ссылка родителю». Отправьте каждому его ссылку. Она одноразовая, действует 48 часов; новая заменяет прежнюю для этой роли.', 'Invitations are available when creating an individual lesson and in the student profile → Invite to my bot: Student link / Parent link. Send each person their own link. Links are single-use, expire in 48 hours and replace the previous link for that role.'],
        ...
        {id: 'messages', title: ['Напоминания, задержки и тексты', 'Reminders, delays and message texts'], items: [
            ['Сообщения ученикам и родителям отправляются через личного бота преподавателя после подтверждения привязки. В карточке ученика включите напоминания ученику и, при необходимости, сообщение родителю об окончании. В уроке задайте время напоминания.', 'Messages are sent through the teacher’s personal bot after the connection is confirmed. Enable student reminders and optional end-of-lesson messages to the parent in the student profile. Set reminder timing in the lesson.'],
        ...
        {id: 'saving', title: ['Сохранение и проблемы соединения', 'Saving and connection problems'], items: [
