...
(() => {
    ...
    const topics = [
        ...
        {id: 'lessons', title: ['Занятия: создание, перенос, отмена', 'Lessons: create, move, cancel'], items: [
            ...
            ['«Перенести»: короткий тап открывает меню, а небольшое движение карточки вбок или по диагонали сразу начинает перенос. Вертикальный свайп прокручивает календарь даже поверх занятия. Быстро отпустите над часом — время станет ровно по началу часа; задержитесь над ним 0,7 секунды — появится шкала с шагом 5 минут.', 'Reschedule: a short tap opens the menu, while a small sideways or diagonal movement immediately starts dragging. A vertical swipe scrolls the calendar even over a lesson. Release quickly over an hour to use its start; stay over it for 0.7 seconds to open the five-minute scale.'],
        ...
        {id: 'invites', title: ['Контакты и привязка к боту', 'Contacts and bot connections'], items: [
            ...
            ['Кнопка «Написать» сначала открывает Telegram. Если его нет — чат WhatsApp по номеру. Если указан только MAX, TEMLI копирует контакт и открывает MAX: вставьте контакт в поиск. Для ученика и родителя эта последовательность работает отдельно.', 'Write opens Telegram first. If it is missing, it opens a WhatsApp chat by phone number. If only MAX is available, TEMLI copies the contact and opens MAX: paste it into search. This priority is applied separately to the student and parent.'],
